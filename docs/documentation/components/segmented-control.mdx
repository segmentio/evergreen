...
> Deprecation note: This component may be removed in future versions of Evergreen. Use Group instead.
...
function BasicSegmentedControlExample() {
  ...
  return <SegmentedControl width={240} options={options} value={value} onChange={(value) => setValue(value)} />
}
```
...
function SmallSegmentedControlExample() {
  ...
  return (
    <SegmentedControl width={240} size="small" options={options} value={value} onChange={(value) => setValue(value)} />
  )
}
```
